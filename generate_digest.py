...
def generate_digest_with_claude(search_results, config):
    ...
    prompt = f"""You are generating a daily email digest covering enterprise intranets, employee experience platforms, consumer website builders, and AI-native tools.
...
<h1>Intranet Daily Digest - {datetime.now().strftime('%B %d, %Y')}</h1>

<p class="greeting">Good morning,</p>
<p class="greeting">Here are today's top updates on enterprise intranets, employee experience platforms, website builders, and AI-native tools.</p>
...
<div class="footer">
    <p>Generated by Kripal's Intranet Daily Digest Agent | Created in Claude Code</p>
...
<head>
    ...
    <title>Intranet Daily Digest - {datetime.now().strftime('%B %d, %Y')}</title>
    <style>
        body {{
...
def main():
    ...
    try:
        log("=" * 60)
        log("Automated Intranet Daily Digest Generator")
        log("=" * 60)
